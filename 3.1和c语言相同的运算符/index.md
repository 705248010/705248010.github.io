...
在C语言中我们已经学过这些术语：`操作数`（operand）、`运算符`（operator）、`左值`（lvalue）和`右值`（rvalue）。

`操作数`（operand）是程序操作的数据实体，该数据可以是数值、逻辑值或其他类型。该操作数既可以是常量也可以为变量。

`运算符`（operator）是可以对数据进行相应操作的符号。如对数据求和操作，用加法运算符 '+'，求积操作使用乘法运算符 '*' 等。

根据运算符可操作的操作数的个数，可把运算符分为`一元运算符`、`二元运算符`和`多元运算符`（一般三元）。

计算机内存中可修改的存储对象，一般称为左值或 lvalue。把可赋值给左值的量称为右值或 rvalue。右值可以是常量、变量或者表达式。

这些基本术语在LPC语言中同样适用，这里不再过多阐述。

我们知道，C语言提供了丰富的运算符，有算术运算符、关系运算符、逻辑运算符、赋值运算符、移位运算符、逗号运算符及 sizeof 运算符。对应有算术表达式、关系表达式、逻辑表达式、赋值表达式、移位表达式、逗号表达式及 sizeof 表达式等。

LPC语言中的运算符和C语言基本相同，但是，在LPC语言中没有以下三个运算符：点运算符`.`、取址运算符`&`和 `sizeof`运算符。注意在LPC语言中的 `sizeof` 只是同名函数，作用是获取数组(array)、映射(mapping)的元素个数或字符串(string)和缓冲(buffer)的长度。
...
上面示例中，在C语言中变量`y`的值还是`-12`，因为`+`不会改变正负值，但**在LPC中会报语法错误**。
...
请注意：以上运算的简形定式在LPC中的某些特殊情况下并不和完整形式完全等效。举个例子：
...
逻辑运算为判断真假，LPC语言和C语言一致：**非0即真**。也就是只有0值（包括int`0`、float`0.0`和字符`'\0'`）为假，其他的全是真。在其他多数高级语言中，空字符串`""`也是假，甚至部分高级语言中空数组和字符串`"0"`也是假，而在LPC语言中这些全部是真，所有注意不要搞出BUG了。
...
注意，LPC语言中逻辑运算的返回结果和C语言不同，并不是返回0（逻辑假）或1（逻辑真），而是和javascript一样，逻辑真时 **&& 的结果是最后一个不为0的值，而 || 的结果是第一个不为0的值**，逻辑假时 **&& 的结果时第一个假值，而 || 的结果时最后一个假值**(反正都是假值0)。 如：`1 && 2 && 3` 返回 3, `0 || 1 || 2` 返回 1。

这样的语法特色可以让代码更为精简，如下代码：
