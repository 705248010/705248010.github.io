...
LPC语言是一门类C语言，其基本语法和C语言一致，为了方便学习这里把C语言的基本语法做整理。以下C语言基本语法在LPC语言中是完全一致的，如果你学过C语言，恭喜你，LPC语言你自动入门了。
...
在C语言中`printf()`是在标准库的头文件`stdio.h`定义的，使用这个函数之前，必须在源码文件头部引入这个头文件。**在LPC语言中没有库函数的说法，只有驱动提供的外部函数，可以直接使用**。
...
`printf()`参数与占位符是一一对应关系，如果有`n`个占位符，`printf()`的参数就应该有`n + 1`个。如果参数个数少于对应的占位符，`printf()`可能会输出内存中的任意值。**注意：在LPC语言中参数个数少于对应占位符会报错**

`printf()`的占位符有许多种类，在LPC语言中的详细用法可参考文档：[printf](https://wiki.mud.ren/index.php?title=Printf)，下面列出常见用法：
